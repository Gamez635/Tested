...
local function updateStatus()
    statusLabel.Text = string.format("IJ:%s | JP:%s(%d) | Sp:%d | Gr:%.1f | CTP:%s | CTW:%s | AB:%s",
        ...
        walkSpeed, customGravity, isClickTP and "On" or "Off", isClickTween and "On" or "Off",
...
local function updateJumpPower()
    ...
    if hum then
        ...
        hum.JumpHeight = isCustomJump and (jumpPower / 1.8) or 7.2
    end
end
...
local function startJumpPowerLoop()
    ...
    jumpPowerLoop = RunService.Heartbeat:Connect(function()
        if isCustomJump then updateJumpPower() end
    end)
end
    ...
    if isAutoBlock then
        autoBlockConnection = RunService.Heartbeat:Connect(function()
            ...
            if not autoBlockPlatform then
                ...
                autoBlockPlatform.Size = Vector3.new(5, 5, 1)
                autoBlockPlatform.Transparency = 0.5
                autoBlockPlatform.BrickColor = BrickColor.new("Really blue")
            ...
            autoBlockPlatform.CFrame = root.CFrame * CFrame.new(0, 0, -4)
        end)
    end
    updateStatus()
end
...
customJumpBtn.MouseButton1Click:Connect(function()
    ...
    if isCustomJump then startJumpPowerLoop() else stopJumpPowerLoop() end
    updateJumpPower()
    updateStatus()
end)
...
updateStatus()
print("✅ Script Loaded with Auto Block Added!")
